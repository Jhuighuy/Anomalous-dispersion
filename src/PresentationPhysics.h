...
public:
    ...
    qreal real(qreal x) const override
    {
        return height() * qExp(-width() * (x - center()) * (x - center()));
    }

private:
    qreal mCenter = 0.58;
    //qreal mWidth = 450.0, mHeight = 0.3;
    qreal mWidth = 1600, mHeight = 0.3;
    /*! @todo Короче, Олег, тут творится какой-то кек. Я просчитал этот 1/e,
     * выставил слайдер как надо, но происходит пиздец.
     * Я вспомнил почему я ограничился именно такой границей узости полосы:
     * Выставь сейчас самую узкую полосу и увидишь у гауссовой функции слева и справа
     * от горба какие-то маленькие хуевинки => на действительной части тоже залупа
     * пойдет. Если выставить еще более узкую полосу, то там их будет прямо видно
     * невооруженным глазом. Попробуй кстати для самой узкой полосы поперемещать ее
     * центр влево вправо и смотри на график. Тебе понравится.
     * Видимо, не хватает точности для расчета экспоненты на таких
     * маленьких величинах, хз. В общем-то хз, но желательно фиксануть, иначе опять
     * будет "бля, сделайте полосу уже"
     * Но с другой стороны, пойдут артефакты на картинке, там полупрозрачные
     * треугольники по сути друг в друге уже будут. Но если мы убираем альфу конуса
     * то это ваще не проблема. Короче, желательно фиксануть эту хуйню с экспонентой.
     * */
};
