		...
		: Window(Rect(), L"Презентация", true)
	{
		...
		// -----------------------
		{
			...
			for (auto cnt = 0; cnt < m_Presentation->m_PrismContollers.size(); ++cnt)
			{
				...
				InitializeModifierControl(prismControl.Angle, L"Угол призмы", prism->Angle, delta, 0, j);
				InitializeModifierControl(prismControl.Rotation, L"Поворот призмы", prism->RotationX, delta, 1, j);
				j++;
#if _DEBUG
